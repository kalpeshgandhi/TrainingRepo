...
<Workflow xmlns="http://soap.sforce.com/2006/04/metadata">
	...
	<rules>
		...
		<formula>AND(NOT(ISBLANK(Phone)), ISBLANK(MobilePhone),DoNotCall
			==FALSE)</formula>
		<triggerType>onCreateOrTriggeringUpdate</triggerType>
	</rules>
	<rules>
		...
		<formula>AND( NOT( ISBLANK(MobilePhone)),(ISBLANK(Phone)),DoNotCall
			=FALSE)</formula>
		<triggerType>onAllChanges</triggerType>
	</rules>
	<rules>
		...
		<active>true</active>
		<description>This is a WorkFlow To Assign the Record Type is No
			Number.</description>
		<formula>AND(( ISBLANK( MobilePhone ) ), (ISBLANK( Phone )), DoNotCall
			==FALSE )</formula>
		<triggerType>onCreateOrTriggeringUpdate</triggerType>
	</rules>
</Workflow>
